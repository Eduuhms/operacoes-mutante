...
describe('Suíte de Testes Fraca para 50 Operações Aritméticas', () => {
  ...
  test('4. deve dividir e lançar erro para divisão por zero', () => {
    ...
    expect(() => divisao(5, 0)).toThrow(/Divis/);
  });
  ...
  test('6b. deve lançar erro para raiz quadrada de número negativo', () => { expect(() => raizQuadrada(-1)).toThrow(/negativo/); });
  test('6c. deve calcular a raiz quadrada de zero', () => { expect(raizQuadrada(0)).toBe(0); });
  ...
  test('8b. deve calcular o fatorial de 0 e 1', () => {
    expect(fatorial(0)).toBe(1);
    expect(fatorial(1)).toBe(1);
  });
  test('8c. deve lançar erro para fatorial negativo', () => { expect(() => fatorial(-3)).toThrow(/Fatorial/); });
  test('9. deve calcular a média de um array com múltiplos elementos', () => { expect(mediaArray([10, 20, 30])).toBe(20); });
  test('9b. deve retornar 0 para média de array vazio', () => { expect(mediaArray([])).toBe(0); });
  ...
  test('11b. deve lançar erro para máximo de array vazio', () => { expect(() => maximoArray([])).toThrow(/Array vazio/); });
  test('12. deve encontrar o valor mínimo em um array', () => { expect(minimoArray([10, 2, 100])).toBe(2); });
  test('12b. deve lançar erro para mínimo de array vazio', () => { expect(() => minimoArray([])).toThrow(/Array vazio/); });
  ...
  test('33b. deve retornar false para 1 e true para 2', () => {
    expect(isPrimo(1)).toBe(false);
    expect(isPrimo(2)).toBe(true);
  });
  test('33c. deve retornar false para número composto', () => { expect(isPrimo(9)).toBe(false); });
  ...
  test('35c. nao deve chamar reduce em array vazio', () => {
    const numeros = {
      length: 0,
      reduce: () => { throw new Error('reduce called'); }
    };
    expect(produtoArray(numeros)).toBe(1);
  });
  test('36. deve manter um valor dentro de um intervalo (clamp)', () => { expect(clamp(5, 0, 10)).toBe(5); });
  test('36b. deve limitar valor abaixo do mínimo', () => { expect(clamp(-5, 0, 10)).toBe(0); });
  test('36c. deve limitar valor acima do máximo', () => { expect(clamp(15, 0, 10)).toBe(10); });
  test('36d. deve preservar +0 quando min e -0', () => {
    const resultado = clamp(0, -0, 10);
    expect(Object.is(resultado, -0)).toBe(false);
  });
  test('36e. deve preservar +0 quando max e -0', () => {
    const resultado = clamp(0, -10, -0);
    expect(Object.is(resultado, -0)).toBe(false);
  });
  ...
  test('38b. deve converter Celsius para Fahrenheit com valor positivo', () => { expect(celsiusParaFahrenheit(100)).toBe(212); });
  test('39. deve converter Fahrenheit para Celsius', () => { expect(fahrenheitParaCelsius(32)).toBe(0); });
  test('39b. deve converter Fahrenheit para Celsius com valor positivo', () => { expect(fahrenheitParaCelsius(212)).toBe(100); });
  test('40. deve calcular o inverso de um número', () => { expect(inverso(4)).toBe(0.25); });
  test('40b. deve lançar erro para inverso de zero', () => { expect(() => inverso(0)).toThrow(/inverter/); });
  ...
  test('44b. deve retornar false quando números são iguais (maior que)', () => { expect(isMaiorQue(5, 5)).toBe(false); });
  test('45. deve verificar se um número é menor que outro', () => { expect(isMenorQue(5, 10)).toBe(true); });
  test('45b. deve retornar false quando números são iguais (menor que)', () => { expect(isMenorQue(5, 5)).toBe(false); });
  test('46. deve verificar se dois números são iguais', () => { expect(isEqual(7, 7)).toBe(true); });
  test('46b. deve retornar false para números diferentes', () => { expect(isEqual(7, 8)).toBe(false); });
  test('47. deve calcular a mediana de um array ímpar e ordenado', () => { expect(medianaArray([1, 2, 3, 4, 5])).toBe(3); });
  test('47b. deve lançar erro para mediana de array vazio', () => { expect(() => medianaArray([])).toThrow(/mediana/); });
  test('47c. deve calcular a mediana de array ímpar não ordenado', () => { expect(medianaArray([5, 1, 3, 2, 4])).toBe(3); });
  test('47d. deve calcular a mediana de array par', () => { expect(medianaArray([1, 2, 3, 4])).toBe(2.5); });
  test('47e. deve calcular a mediana de array ímpar não ordenado com dois dígitos', () => { expect(medianaArray([10, 2, 3])).toBe(3); });
  test('47f. deve calcular a mediana de array par não ordenado com dois dígitos', () => { expect(medianaArray([10, 2, 4, 8])).toBe(6); });
